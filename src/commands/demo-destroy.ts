...
const destroyDemo = async () => {
  try {
    console.log(chalk.blue.bold('\n🗑️  Demo Stack - Complete Teardown\n'));
    ...
    const { confirmTeardown } = await inquirer.prompt([
      {
        ...
        message:
          'This will delete BOTH demo stacks (Vispyr backend + demo app), EC2 instances, VPCs, NAT Gateways, Elastic IPs, CDKToolkit stack, and bootstrap S3 bucket. Continue?',
        default: false,
      },
    ]);
    ...
    // 1. Check for any unattached Elastic IPs before destroying stacks
    ...
    // 2. Destroy Demo App Stack (DemoStack)
    const demoDestroySpinner = ora(
      'Destroying demo app stack (Node.js + PostgreSQL, VPC, etc.)...'
    ).start();
    try {
      const { stdout, stderr } = await execAsync(
        'npx cdk destroy DemoStack --force'
      );
      demoDestroySpinner.succeed('Demo app stack destroyed successfully');
      ...
      demoDestroySpinner.fail('Failed to destroy demo app stack');
      console.error(chalk.red(error));
      // Continue with other cleanup even if stack destruction fails
    }

    // 3. Destroy Vispyr Backend Stack (Ec2Stack)
    const backendDestroySpinner = ora(
      'Destroying Vispyr backend stack (Grafana, VPC, NAT Gateway, etc.)...'
    ).start();
    try {
      const { stdout, stderr } = await execAsync(
        'npx cdk destroy Ec2Stack --force'
      );
      backendDestroySpinner.succeed(
        'Vispyr backend stack destroyed successfully'
      );
      ...
      backendDestroySpinner.fail('Failed to destroy Vispyr backend stack');
      console.error(chalk.red(error));
      // Continue with other cleanup even if stack destruction fails
    }
    ...
    try {
      ...
      if (fs.existsSync('backend-outputs.json'))
        fs.removeSync('backend-outputs.json');
      if (fs.existsSync('demo-outputs.json'))
        fs.removeSync('demo-outputs.json');
    ...
    console.log(chalk.green.bold('\n✅ Complete demo teardown finished!'));
    console.log(
      chalk.gray('Both demo stacks and all AWS resources have been cleaned up.')
    );
    console.log(
      chalk.gray(
